...
namespace Ajsuth.Sample.OrderCloud.Engine.Controllers
{
    ...
    public class CommerceOpsController : CommerceODataController
    {
        ...
        public async Task<IActionResult> ExportToOrderCloud([FromBody] ODataActionParameters value)
        {
            ...
            var result = await command.Process(CurrentContext, processSettings, (value["siteSettings"] as IEnumerable<SitePolicy>)?.ToList(), value["userSettings"] as UserPolicy, value["productSettings"] as SellableItemExportPolicy).ConfigureAwait(false);
