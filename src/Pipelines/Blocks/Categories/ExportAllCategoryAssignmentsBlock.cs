// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ExportAllCategoryAssignmentsBlock.cs" company="Sitecore Corporation">
...
{
    /// <summary>Defines the asynchronous executing ExportAllCategoryAssignments pipeline block</summary>
    /// <seealso cref="AsyncPipelineBlock{TInput, TOutput, TContext}" />
    [PipelineDisplayName(OrderCloudConstants.Pipelines.Blocks.ExportAllCategoryAssignments)]
    public class ExportAllCategoryAssignmentsBlock : AsyncPipelineBlock<ExportToOrderCloudArgument, ExportToOrderCloudArgument, CommercePipelineExecutionContext>
    {
        /// <summary>Gets or sets the commander.</summary>
        /// <value>The commander.</value>
        protected CommerceCommander Commander { get; set; }

        /// <summary>Initializes a new instance of the <see cref="ExportAllCategoryAssignmentsBlock" /> class.</summary>
        /// <param name="commander">The commerce commander.</param>
        public ExportAllCategoryAssignmentsBlock(CommerceCommander commander)
        {
        ...
        public override async Task<ExportToOrderCloudArgument> RunAsync(ExportToOrderCloudArgument arg, CommercePipelineExecutionContext context)
        {
            ...
            if (!arg.ProcessSettings.ProcessCategoryAssignments)
            {
                context.Logger.LogInformation($"Skipping category assignments export - not enabled.");
                return arg;
            }
            ...
            context.Logger.LogInformation($"{Name}-Exporting category assignments Completed: {(int)itemsProcessed}. Environment: {context.CommerceContext.Environment.Name}");
            return arg;
        }
