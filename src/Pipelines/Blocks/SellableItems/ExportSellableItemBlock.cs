...
namespace Ajsuth.Sample.OrderCloud.Engine.Pipelines.Blocks
{
    ...
    public class ExportSellableItemBlock : AsyncPipelineBlock<SellableItem, SellableItem, CommercePipelineExecutionContext>
    {
        ...
        public override async Task<SellableItem> RunAsync(SellableItem sellableItem, CommercePipelineExecutionContext context)
        {
            ...
            var processSettings = exportSettings.ProcessSettings;
        ...
        {
            foreach (var siteSettings in SiteSettings)
            {
                var storefront = context.CommerceContext.GetEntity<Shop>(Shop => Shop.Name.EqualsOrdinalIgnoreCase(siteSettings.Storefront));
                foreach (var currency in storefront.Currencies)
                {
                    var priceSchedule = priceSchedules.FirstOrDefault(p => p.Currency.EqualsOrdinalIgnoreCase(currency.Code));
                    ...
                    catch (Exception ex)
                    {
                        ...
                        context.Logger.LogError($"Saving product assignment failed; Product ID: {product.ID}, Buyer ID: {buyerId}, User Group ID: {userGroupId}\n{ex.Message}\n{ex}");
                    }
                }
            }
        }
